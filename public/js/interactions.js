(() => {
  ...
  async function renderInteractions(typ) {
    ...
    for (const [i, item] of items.entries()) {
      ...
      const imgSrc = "/image_proxy/" + encodeURIComponent(item.actor.icon) + "/" + typ + "/" + encodeURIComponent(item.id);
      ...
      if (i == 3) {
        previewImages.appendChild(document.createTextNode("\xA0\u2026"));
      }
  ...
  async function main() {
    return Promise.all[renderInteractions("likes"), renderInteractions("shares")];
  }
  main();
})();
